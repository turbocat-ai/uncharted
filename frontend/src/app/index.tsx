...
import { StyleSheet, View, Text, TouchableOpacity, SafeAreaView, Platform, ViewStyle, TextStyle, TextInput } from 'react-native';
...
const IS_DEBUG = process.env.EXPO_PUBLIC_DEBUG_MODE === 'true' || __DEV__;

// Default starting point (e.g., NYC) if no real GPS location is available yet
...
export default function MapScreen() {
  ...
  // Helper to handle coordinate updates (both real GPS and Mocked GPS)
  ...
  // Request real location permissions (Skipped if debugging without real GPS)
  useEffect(() => {
    (async () => {
      if (IS_DEBUG) {
        // Fallback default coordinates for mock tool
        const mockLoc = createMockLocationObject(DEFAULT_DEBUG_COORDS.latitude, DEFAULT_DEBUG_COORDS.longitude);
        setLocation(mockLoc);
        return;
      }
  ...
  // Real GPS Streaming (Only active when IS_DEBUG is false)
  useEffect(() => {
    if (IS_DEBUG) return; // Skip real GPS listener in debug mode
        ...
        try {
          const sub = await Location.watchPositionAsync(
            ...
            async (newLocation) => {
              ...
              await processNewCoordinates(newLocation.coords.latitude, newLocation.coords.longitude);
            }
          );
        ...
        <View style={styles.debugPanel}>
          ...
          <View style={styles.debugBtnRow}>
            <TouchableOpacity
              ...
              onPress={() => handleMockMove(0.002, 0)} // Move North ~220m
            >
              <Text style={styles.debugBtnText}>↑ North</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.debugBtn}
              onPress={() => handleMockMove(-0.002, 0)} // Move South
            >
              <Text style={styles.debugBtnText}>↓ South</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.debugBtn}
              onPress={() => handleMockMove(0, 0.002)} // Move East
            >
              <Text style={styles.debugBtnText}>→ East</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.debugBtn}
              onPress={() => handleMockMove(0, -0.002)} // Move West
            >
          ...
          <View style={styles.manualInputRow}>
            ...
            />
            <TouchableOpacity style={styles.debugSetBtn} onPress={handleManualCoordinateSubmit}>
