...
// Add an init lock to prevent async race conditions
let isInitializing = false;

export async function initUserDatabase(userId?: number): Promise<SQLite.SQLiteDatabase> {
  // 1. Fallback to Dev User ID if bypassed login left userId undefined/null
  const activeUserId = userId ?? (__DEV__ ? 999 : 0);

  if (activeUserId === 0 && !__DEV__) {
    throw new Error('[DB] Cannot initialize database without a valid userId in production.');
  }

  // 2. Return existing DB connection if already loaded for this user
  if (currentDb && currentUserId === activeUserId) {
    return currentDb;
  }

  // 3. Simple spin-lock to prevent race conditions during rapid re-renders
  while (isInitializing) {
    await new Promise((resolve) => setTimeout(resolve, 50));
    if (currentDb && currentUserId === activeUserId) return currentDb;
  }
  ...
  try {
    ...
    const db = await SQLite.openDatabaseAsync(`user_${activeUserId}.db`);
...
*/
// Simple in-memory lock chain to serialize concurrent database writes
let writeLock: Promise<unknown> = Promise.resolve();

/**
 * Public function called by GPS listeners / spatial hooks.
 * Queues execution sequentially to eliminate "cannot start a transaction within a transaction" errors.
 */
export async function logHexVisit(h3Index: string): Promise<boolean> {
  return new Promise((resolve, reject) => {
    writeLock = writeLock.then(async () => {
      try {
        const result = await executeLogHexVisit(h3Index);
        resolve(result);
      } catch (err) {
        reject(err);
      }
...
/**
 * Internal worker function executing the SQLite transaction.
 */
async function executeLogHexVisit(h3Index: string): Promise<boolean> {
